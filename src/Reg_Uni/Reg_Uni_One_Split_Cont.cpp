                        ...
                        bool useobsweight)
{
  ...
  cout << "Inside Split: "
       << "sum(Y): " << sum(Y) << "size(Y): " << size(Y) << endl;
  cout << "nsplit: " << nsplit << endl;
  cout << "split_gen: " << split_gen << endl;

  if (split_gen == 1) // random split
  {
    for (int k = 0; k < nsplit; k++)
    {
      cout << "split trying number " << k << endl;
      ...
      cout << "useobsweight: " << useobsweight << "temp_score " << temp_score << "TempSplit.score" << TempSplit.score << endl;
  ...
  {
    // move index if ties
    while (x(indices(lowindex)) == x(indices(lowindex + 1)))
      lowindex++;
    while (x(indices(highindex)) == x(indices(highindex + 1)))
      highindex--;
